...
if [[ -e "${PROJECT_ROOT}/vendor/heycart/platform" ]]; then
    STOREFRONT_ROOT="${STOREFRONT_ROOT:-"${PROJECT_ROOT}/vendor/heycart/platform/src/Storefront"}"
else
    STOREFRONT_ROOT="${STOREFRONT_ROOT:-"${PROJECT_ROOT}/vendor/heycart/storefront"}"
fi
...
[[ ${HEYCART_SKIP_FEATURE_DUMP:-""} ]] || "${BIN_TOOL}" feature:dump
...
[[ ${HEYCART_SKIP_THEME_COMPILE:-""} ]] || "${BIN_TOOL}" theme:compile --active-only
