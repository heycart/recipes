...
if [[ -e "${PROJECT_ROOT}/vendor/heycart/platform" ]]; then
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/heycart/platform/src/Administration"}"
else
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/heycart/administration"}"
fi
...
[[ ${HEYCART_SKIP_ASSET_COPY:-""} ]] ||"${BIN_TOOL}" assets:install
