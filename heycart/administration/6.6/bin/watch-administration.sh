...
[[ ${HEYCART_SKIP_BUNDLE_DUMP:-""} ]] || "${BIN_TOOL}" bundle:dump
...
if [ ! -d vendor/heycart/administration/Resources/app/administration/node_modules/webpack-dev-server ]; then
    npm install --prefix vendor/heycart/administration/Resources/app/administration/
fi

# Dump entity schema
if [[ -z "${HEYCART_SKIP_ENTITY_SCHEMA_DUMP:-""}" ]] && [[ -f "${ADMIN_ROOT}"/Resources/app/administration/scripts/entitySchemaConverter/entity-schema-converter.ts ]]; then
...
npm run --prefix vendor/heycart/administration/Resources/app/administration/ dev
